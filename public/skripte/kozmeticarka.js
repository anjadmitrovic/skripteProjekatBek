...
function getAll() {
  fetch('http://localhost:8000/admin/kozmeticarka', {
    method:'GET',
    headers:{
      ...
      if(!data.msg){
        data.forEach(element => {
          ...
          const text = document.createTextNode(
            `${element.ime} ${element.prezime} ${element.vrstaUsluge}`
          )
...
document.getElementById("add").addEventListener("click", (e)=>{
  ...
  const vrstaUsluge = document.getElementById("vrstaUsluge").value
